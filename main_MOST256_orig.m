...
load sigMatHair 
sigMat = [zeros(205,256) ;sigMat(200:end,:)];
n = 128;
xsize=[n,n];

c = 1444;  % speed of sound [m/s]
image_width = 17e-3;                                % width of the image
dx=image_width/(n-1);
...
r_sensor = 39e-3;                                 % distance of the transducer from the centre of rotation [m]
fs=4e7;
...
%% Build model matrix
if exist(['Amat' num2str(n) '_' num2str(n_proj) '.mat'])
    load (['Amat' num2str(n) '_' num2str(n_proj) '.mat'])
else
    A_mat = Calculate_MatrixMB_Luis(c,n,image_width,t,r_sensor,angle_sensor,n_angles);
    save(['Amat' num2str(n) '_' num2str(n_proj) '.mat'],'A_mat')
